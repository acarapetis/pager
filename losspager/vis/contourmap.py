...
def _get_open_corner(popgrid, ax, filled_corner=None, need_bottom=True):
    ...
    lrpopsum = popdata[
        nrows - int(nrows / 4) : nrows - 1, ncols - int(ncols / 4) : ncols - 1
    ].sum()
...
def draw_contour(
    ...
    oceangrid = read(oceangridfile, samplegeodict=gd, resample=True, doPadding=True)
    ...
    if gd.xmin < gd.xmax:
        ...
        xmin, xmax, ymin, ymax = (gd.xmin, gd.xmax, gd.ymin, gd.ymax)
        xmax += 360
        width = ((gd.xmax + 360) - gd.xmin) * np.cos(np.radians(center_lat)) * DEG2KM
    ...
    if borderfile is not None:
        borders = ShapelyFeature(Reader(borderfile).geometries(), ccrs.PlateCarree())
        ax.add_feature(
            borders,
            zorder=COAST_ZORDER,
            edgecolor="black",
    ...
    ax2.add_feature(
        cfeature.OCEAN, zorder=0, facecolor=WATERCOLOR, edgecolor=WATERCOLOR
    )
    ...
    for axis in ["top", "bottom", "left", "right"]:
        ...
        ax2.spines[axis].set_edgecolor("black")
